    ...
    Ebr<T, SLOTS, BUMP_EPOCH_OPS>
{
    ...
    pub fn pin(&self) -> Guard<'_, T, SLOTS, BUMP_EPOCH_OPS> {
        ...
        if inner.local_concurrent_pins == 1 {
            let mut global_current_epoch = inner.global_current_epoch.load(Ordering::SeqCst);
            ...
            loop {
                inner
                    ...
                    .access_without_notification(|lqe| {
                        lqe.store(global_current_epoch, Ordering::SeqCst);
                    });
                ...
                let second_read = inner.global_current_epoch.load(Ordering::SeqCst);
    ...
    for Inner<T, SLOTS, BUMP_EPOCH_OPS>
{
    fn drop(&mut self) {
        ...
        if self.current_garbage_bag.len > 0 {
            ...
            let mut global_current_epoch = self.global_current_epoch.load(Ordering::SeqCst);
            ...
            loop {
                ...
                let second_read = self.global_current_epoch.load(Ordering::SeqCst);
    ...
    Inner<T, SLOTS, BUMP_EPOCH_OPS>
{
    ...
    fn bump_quiescent(&mut self) {
        ...
        let last_epoch = self.global_current_epoch.fetch_add(2, Ordering::SeqCst);
        ...
        let minimum_fold_closure = |min: u64, this: &AtomicU64| {
            let value = this.load(Ordering::SeqCst);
        ...
        let fetch_max_response = self
            ...
            .fetch_max(global_minimum_epoch, Ordering::SeqCst);
    ...
    fn clean_up(&mut self) {
        let global_minimum_epoch = self.global_minimum_epoch.load(Ordering::SeqCst);
    ...
    fn drop(&mut self) {
        ...
        if inner.local_concurrent_pins == 0 {
            inner
                .local_progress_registry
                .access_without_notification(|lqe| {
                    let global_minimum_epoch = inner.global_minimum_epoch.load(Ordering::SeqCst);
                    let last = lqe.fetch_or(INACTIVE_BIT, Ordering::SeqCst);
    ...
    Guard<'a, T, SLOTS, BUMP_EPOCH_OPS>
{
    pub fn defer_drop(&mut self, item: T) {
        ...
        let mut global_current_epoch = ebr.global_current_epoch.load(Ordering::SeqCst);
        ...
        loop {
            ...
            let second_read = ebr.global_current_epoch.load(Ordering::SeqCst);
